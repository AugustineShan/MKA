...
def test_detect_prepaid_sign_normalize_flips_when_total_cogs_includes_impair():
    """Opt 4: pre-2019 assets_impair_loss 在 raw total_cogs 内（旧口径损失记正号）
    → 翻符号。新乳业 2018 形态：raw total_cogs 4782.87 − adapted 4772.38 = 10.49 = impair。"""
    import pandas as pd
    wide = pd.DataFrame({"total_cogs": [4772.38], "assets_impair_loss": [10.49]}, index=["2018"])
    failures = [ar.Failure(period="2018", code="IS 1.2", statement="income", title="营业利润",
                           message="IS 1.2 2018 ...", residual=16.97, target_value=257.97,
                           calc_value=241.00, direction="target_gt_calc")]
    overrides = ar.detect_prepaid_sign_normalize(wide, {}, failures, "002946.SZ", {"2018": 4782.87})
    assert len(overrides) == 1
    o = overrides[0]
    assert (o["field"], o["period"]) == ("assets_impair_loss", "2018")
    assert o["old_value_million_cny"] == 10.49
    assert o["new_value_million_cny"] == -10.49
    assert round(o["delta_million_cny"], 2) == -20.98
    assert o["source"] == "glm"  # clean.py applies source=glm approved overrides


def test_detect_prepaid_sign_normalize_skips_when_total_cogs_excludes_impair():
    """Opt 4: total_cogs 不含 assets_impair_loss（raw−adapted≈0）→ 不翻。
    会稽山 2018 形态：+5.36 正确，翻符号会破坏。这正是 standalone 规则回退的根因——
    跨年符号约定不一致，必须用 raw−adapted 逐年判定而非按年份一刀切。"""
    import pandas as pd
    wide = pd.DataFrame({"total_cogs": [4772.38], "assets_impair_loss": [5.36]}, index=["2018"])
    failures = [ar.Failure(period="2018", code="IS 1.2", statement="income", title="营业利润",
                           message="IS 1.2 2018 ...", residual=12.0, target_value=0.0,
                           calc_value=0.0, direction="target_gt_calc")]
    overrides = ar.detect_prepaid_sign_normalize(wide, {}, failures, "601579.SH", {"2018": 4772.38})
    assert overrides == []


def test_detect_prepaid_sign_normalize_skips_post_2019_and_non_is12():
    """Opt 4: 2019+ 新准则 assets_impair_loss 已负号不翻；非 IS 1.2 失败不翻。"""
    import pandas as pd
    wide = pd.DataFrame({"total_cogs": [4772.38, 4772.38], "assets_impair_loss": [10.49, 10.49]},
                        index=["2023", "2018"])
    failures = [
        ar.Failure(period="2023", code="IS 1.2", statement="income", title="营业利润",
                   message="IS 1.2 2023 ...", residual=55.14, target_value=507.9,
                   calc_value=452.76, direction="target_gt_calc"),
        ar.Failure(period="2018", code="BS 2.2", statement="balancesheet", title="非流动资产",
                   message="BS 2.2 2018 ...", residual=1180.99, target_value=0.0,
                   calc_value=0.0, direction="target_gt_calc"),
    ]
    overrides = ar.detect_prepaid_sign_normalize(wide, {}, failures, "002946.SZ",
                                                  {"2023": 4782.87, "2018": 4782.87})
    assert overrides == []  # 2023 post-2019 skipped; 2018 BS 2.2 not IS 1.2 skipped
