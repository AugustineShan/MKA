...
默认规则：BRKD、LOAD、KA 收窄为利润表 + 业务层盈利模型。`financial expense`、`EBIT`、`DA`、`CAPEX`、`CWC`、`shares`、`WACC` 等不主动变成预测旋钮。

例外按顺序判断：

1. `financial expense` 若来自现金、债务、利率或 BS 推导，交引擎或专门流程；只有材料明确给出“其他财务费用”这类利润表外生项时，才走 `other_fin_exp_abs`。
