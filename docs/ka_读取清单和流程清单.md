...
| 4 | markdown 化最高权重材料 | `py -m src.ka_prepare "{公司}"`，幂等转 `公司判断和最新观点.md` + 最高权重材料顶层 → `markdown存储区\`；后续只读存储区 + manifest | SKILL.md §4 |
...
| 7 | 读 reference 候选 + 门禁 | 扫 KA 参考稿区 `核心假设参考*.md`（剔除 load）；**BRKD/已完成 LOAD/KA 参考稿区 reference 三者至少一，否则停止** | SKILL.md §6b |
...
| 段 | 做什么 | 编辑器 § |
|---|---|---|
| 输入权重 + defaults 审计 | 按权重排冲突（最高权重 > BRKD > LOAD > reference > /init > 年报）；**必读 `Agent/defaults.yaml`**，摘 `base_period`、关键参数 `value/source/method`、顶层 `review_flags` | §1 / §1.1 |
...
| 文件 | 角色 |
|---|---|
| `companies\{公司}\公司判断和最新观点.md` | **最高权重材料默认项**（分析师当前 thesis、口径、偏好）；经 ka_prepare markdown 化后读存储区 |
| `…\Skills素材包\最高权重材料-放Agent最应对齐的材料\` 顶层材料 | 最高权重材料其余项；同样只读 `markdown存储区\` + manifest，不读 raw |
...
| KA 参考稿区 `核心假设参考*.md`（剔除 load） | reference 候选（brkd/alphapai，含 Alphapai-load），只作候选理解，预测值必须重裁 | #4 |

> 门禁底线（SKILL.md §6b）：最高权重材料、旧正式稿、`公司判断和最新观点.md` **不计入此门禁**——它们是裁决材料或旧稿对照，不是业务骨架来源。
...
| 命令 | 时机 |
|---|---|
| `py -m src.ka_prepare "{公司}"` | §4，幂等 markdown 化最高权重材料 + `公司判断和最新观点.md` |
...
- **编辑器 §0**：只有最高权重材料或旧正式稿、没有 BRKD 也没有完成 LOAD → 停，不凭空生成业务骨架。
