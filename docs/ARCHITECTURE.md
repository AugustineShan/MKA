...
**建模技能管线**：取数流水线之外，业务理解层由多个 skill 协同，而不是一个 Agent 通吃。非标投研材料先进入 markdown staging，再进入候选理解或裁决。`/load` 只读 `Skills素材包/LOAD外部EXCEL模型理解器（一次最多一个）/` 的唯一 Excel，建立 load-vintage 时间沙箱，主产物写公司根目录 `{原Excel文件名}_核心假设_load{YYYYMMDD}.md`，并同步副本到 `Agent/Load/{load_id}/`；`/brkd` 先把 `Skills素材包/BRKD业务理解器（研报和纪要放在这里）/` 幂等转为 `markdown存储区/`，再结合 `/init` 历史事实和年报生成 `Agent业务讨论.md`，若另存核心假设式 draft 则必须命名 `*_核心假设_brkd{YYYYMMDD}.md`；`docs/Alphapai/Alphapai业务拆分抓取器.md` 是网页端 factpack prompt，只抓用户指定主拆分、桥表和副拆分历史，不写预测；Alphapai-load 的 `核心假设参考.md` 是 root reference 候选，若存在业务拆分 factpack 必须优先承接；`/ka` 不读原始 Excel/研报/纪要，只裁决同权重判断材料、BRKD、LOAD、reference 候选和 `/init` 校验层，生成正式 `核心假设.md`；`/comp` 忠实翻译为 `yaml1`，通过信息保全/audit 后跑 forecast。已有正式稿的调整交给 `/adj`、`/frontend-edit` 或 `/annual-update`。
...
当前网页端重活只保留 `/webload`：旧 Excel 模型 vintage 保存必须先锁时间沙箱，再在网页端完成模型理解 overview 和分段确认。普通 `/ka` 现在是本地全量裁决器，直接读取同权重判断材料、BRKD、LOAD/reference 和 `/init` 校验层，不再提供网页端打包入口。
...
与 `/load` 的区别：
- `/load` 是真正的模型装载流程，按 `/ka` 的会议纪律先 overview、再分段确认、最后写 `{原Excel文件名}_核心假设_load{YYYYMMDD}.md`；`/load` 止于此 markdown，不编译 yaml1、不跑 DCF。
...
- 网页端产出的 `{原Excel文件名}_核心假设_load{YYYYMMDD}.md` 放回 `Agent/Load/{load_id}/` 与 KA 参考稿区后，`/load` 到此为止；要变成当前正式 forecast 另走 `/ka` → `/comp`。
...
**Business Fact Matrix 契约（2026-06-29）**：`src.yaml1_business_facts.build_business_fact_view()` 是 `yaml1` 非标拆分到前端展示的 canonical 边界。`GET /api/companies/{id}` 同时返回旧 `yaml1_revenue_view` 和新 `yaml1_business_facts_view`；前端优先用新视图渲染业务拆分，旧视图只作 fallback。`yaml1` 可以继续保持非标形状，但进入 workbench 后必须标准化为 `BusinessFactBlock -> BusinessFactRow(entity, metric, values, source, editable_path)`。`display` 只决定 role/placement/dimension，指标语义、别名、格式和 fallback 由 `src/business_metric_registry.yaml` 管理。毛利率等派生指标优先使用直存值，只有直值缺失才走 registry 声明的 fallback；未知指标必须保留为 `custom:*` 并 warning，不能静默丢弃。
...
| IS 1.2 确定性联合补缺（LLM-free，先于 LLM 批处理） | `_is12_deterministic_joint_fill` + `_extract_section_item_raw` + `_detect_income_statement_unit` | IS 1.2 残差常是多个 TuShare-NULL optional 字段（oth_income/credit_impa_loss/asset_disp_income）的**净值**，GLM 单轮提议易漏字段+带符号求和算术错（绿联 2025：三字段净值 2.6225，GLM 只提两项却声称和=2.62），trial-apply 挡下后无重试→exit 3→subagent。新增确定性路径：`_llm_propose_fallback` 在 LLM 批处理前对 IS 1.2 残差读合并利润表整段、抽取所有 NULL optional 字段年报非零值、整组 trial-apply（check oracle 裁判），闭合即产 override 跳过 LLM（`source=glm`/`approved_by=glm:is12_joint_fill`）。`_extract_section_item_raw` 三点：① 折行标签合并（"信用减值损失（...号填"/"列）"→一行）；② 短(≤4)无数字行=折行后缀跳过、长无数字行=下一科目标签停止（防空科目"净敞口套期收益"抓下一科目"公允价值变动收益"值）；③ 不跳 <TOLERANCE 小值以纳入 asset_disp_income -0.0273 真实小额。`_detect_income_statement_unit` 用 revenue 锚点定元/千元。trial-apply 是闭合唯一裁判，提取错值/抓错列必不闭合，不会脏配平。不闭合返回 [] 退回 LLM 路径 |
