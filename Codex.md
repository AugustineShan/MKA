...
> **STOP：先判斜杠路由，再选择任何外部技能。** 在 `D:\MKA` 项目里，`/ka 百润股份`、`/ka 002568`、`/comp 002946`、`/init 新乳业` 这类命令一律先解释为 MKA 任务路由；即使公司名或股票代码像证券问题，也不是行情查询、不是交易建议、不是通用 A 股分析、不是 shell。只有用户明确说“查行情/股价/涨停/分时/盘面/资金”时，才可以调用股票行情分析能力。
...
路由冲突防呆：在本项目里，`/ka 百润股份`、`/comp 002568`、`/init 新乳业` 这类以斜杠开头的短命令，优先解释为 MKA 路由，不是行情查询、不是交易建议、不是通用 A 股分析，也不是 shell 命令。只有用户明确说“查行情/股价/涨停/分时/盘面/资金”时，才可以离开 MKA 路由去调用行情分析能力。

当用户说 `/ka 新乳业`、`/comp 002946`、`/annual-update 影石创新` 时，Codex 应该：

1. 把斜杠词识别为 MKA skill 名称，而不是 shell 命令、行情查询或普通股票分析。
2. 先读 `D:\MKA\Codex.md` 和 `D:\MKA\docs\技能简要分类.md` 做分流；分不清规则边界时再读 `D:\MKA\docs\MKA规则导航图.md`。
...
-> Agent业务讨论.md / KA 目录顶层 markdown（核心假设参考*.md 或信息指引）
...
源语言语法看 `docs/核心假设源语言语法规范.md`；`/comp` 翻译账本看 `docs/核心假设翻译IR契约.md`。前者规范 official/candidate markdown 形状，后者规范 Semantic IR、A/B/C 分类和 audit 六段。若不知道该读哪份契约，先看 `docs/MKA规则导航图.md`。

### 1.3 人工筛选门

MKA 不把所有 markdown cache 都当作判断材料。人工筛选是第一门：只有被人放入同权重判断材料、`Agent业务讨论.md`、KA 目录顶层全部 markdown、已完成 LOAD 主产物、Alphapai reference/factpack 或 `/init` 事实索引的内容，才进入本轮 `/ka` 裁决。

KA 目录固定是 `companies\{公司}\Skills素材包\KA（ALPHAPAI拆出来的东西放在这里）\`。丢进这里的顶层 `*.md` 就是给 `/ka` 看的：`核心假设参考*.md` 或带 reference 状态的文件按候选稿裁决；其他 markdown 按信息指引读取，不要求 `待 /ka 裁决清单`，也不自动晋升 official。

`markdown存储区/`、`WEBCLAUDE/` 打包目录、`Agent/Load/` 沙箱副本、临时转换件和历史 cache 默认不是证据入口，只是中间产物；看见 markdown 不等于必须吸收。除非用户明确说“这份材料进入本轮判断”，否则 Codex 不主动扩读、不把 cache 晋升为证据入口。

入口窄，收纳宽：人工筛选门只限制读取范围；对已进入本轮的材料，有复盘价值但暂不入模的信息宁可进收纳区/stash，不得为了减负乱扔。
...
- `/ka` 是唯一裁决器：把人工筛选入口中的候选 markdown 和同权重判断材料裁成 official 人话核心假设，不写 yaml1。
