...
def collect_failures(
    ...
    *,
    ...
    for period in sorted(str(period) for period in wide.index.tolist()):
        ...
        messages: list[str] = []
        # Mirror clean.validate_wide: resolve sign-questionable field signs per-year
        # (regime detected from raw_total_cogs in attrs) and pass sign_map to
        # check_is. Without this, pre-2019 years with impair would show an inflated
        # +2×impair residual (the old 符号坑) and the reconciler would propose
        # wrong overrides. clean.py now closes those years itself, so reconciler
        # only fires on genuine NULL gaps — but the residual it sees must match
        # clean.py's.
        ...
        messages.extend(clean.check_is(row, present, period, sign_map=sign_map, null_fields=null_fields))
...
def main(argv: list[str] | None = None) -> int:
    ...
    # 捕获 adaptation 前的 raw total_cogs（adaptation 会把 total_cogs 改成明细和、剔除
    # impair）。resolve_is_signs 靠 raw_total_cogs − 稳定明细和 判定 sign-questionable 字段
    # 是否在 total_cogs 内（旧口径 regime）；存 attrs 供 collect_failures 镜像 clean.validate_wide。
