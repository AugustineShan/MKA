...
from src.revenue_fold import (
    Yaml1CleanError,
    margin_values as _margin_values,
    product as _product,
    project_leaf,
    projection_values as _projection_values,
    require_mapping as _require_mapping,
    to_float as _to_float,
    unit_factor as _unit_factor,
    year_values as _year_values,
)
...
# Yaml1CleanError is imported above from src.revenue_fold and re-exported here,
# so existing `from src.yaml1_cleaner import Yaml1CleanError` callers keep the
# exact same exception class identity.
...
def _fold_revenue_leaf(
    ...
    # Per-family revenue math lives once in src.revenue_fold.project_leaf; this
    # only packs the leaf projection into the cleaner's _RevenueFold container.
    proj = project_leaf(name, segment, path, inherited_note, horizon, warnings, formula_result)
    return _RevenueFold(
        base_revenue=proj.base_revenue,
        revenue_by_year=proj.revenue_by_year,
        segment_base_revenue={name: proj.base_revenue},
        segment_revenue_by_year={name: proj.revenue_by_year},
        unit_factors={name: proj.unit_factor},
        margin_by_leaf={name: proj.margin} if proj.margin is not None else {},
    )
