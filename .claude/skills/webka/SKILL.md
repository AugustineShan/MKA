...
4. 强制两道门禁（与 /ka 一致，硬停）：
   ...
   - **§6b 骨架门禁**（档定义与 /ka §6b 一致）：(1) BRKD 档=公司根目录 `Agent业务讨论.md`；(2) LOAD 档=KA 目录 `核心假设参考load_*.md`（须有 ` ```knobs` 块且非「待模型装载器补全」脚手架——本打包器只校验这 2 条，/ka §6 另有抬头/路径排除 3 条由本地 /ka 最终把关）；(3) KA 目录顶层任一 markdown（含 `核心假设参考brkd_*.md`，归此档不计 BRKD 档）。三者全无 → 停。
5. 清空并重建：
...
- 读取顺序：`readme first.md` → `必读和素材.md` 全读 → `速查参考.md` 碰到才查。
