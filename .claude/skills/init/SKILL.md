...
> **IS 1.2 多 NULL 字段族已自动闭合（2026-06-29）**：reconciler 的 `_is12_deterministic_joint_fill` 现在在 LLM 批处理前确定性闭合"残差=多个 TuShare-NULL optional 字段（oth_income/credit_impa_loss/asset_disp_income 等）净值"的 IS 1.2 残差（读合并利润表填全 NULL 字段、整组 trial-apply），不再触发 exit 3。绿联 2025 这类 case 今后 round-1 即过。subagent 升级通道仍保留给其它结构性案例（BS 字段映射飘、compound 残差、CF 7.4 重述等）。
