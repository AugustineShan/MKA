...
`/annual-update` 完整继承核心纪律 A1-A7，编辑同一套核心假设源语言 B，滚动旧稿和重吐 `knobs` 块时以 `docs/knobs块契约.md` 为准。本文只保留年度更新独有条款：旧稿只读另存、外层 `/init` 刷数、声明式估算、双向核、annual_update_fetcher 取数清单。

交互风格继承核心纪律 A4：年度更新的人机段要像复盘会，不像滚表日志。先输出你对真实数据、旧预测偏离、thesis 兑现度和建议重拨范围的理解，再等分析师确认；聊天里用会议 memo，落盘稿和 `knobs` 仍完整写文件。
...
2. **读最新旧稿**:Glob 非递归 `companies\{公司}\核心假设*.md`(只认根目录,不认 `WEBCLADE\` 等子目录),取时间最新一份。从抬头提取:
   ...
   - 显式期 / 衰减期 / 衰减交接增速 `terminal.fade.target_growth` / 永续点(第2步时间轴平移要用)。
...
- **第2步「标准线填历史」(自动)**:用第3步的 JSON `lines` 把 (H, A] 实际值 append 进旧稿各段:
  ...
  - **滚后时间轴确认门**:在任何估算或未来重定前,先向分析师报告旧稿四数、数据实际到达年 A、N 年移动、滚后四数、显式期新增末年和已知拐点覆盖情况。分析师未确认前,不得进入估算、不得重定未来、不得写新日期核心假设。
- **第3步「估算拿不到的」(和你一起)**:非标业务线原子(销量/吨价/分线收入)走口径阶梯——
  ...
  - 先押再问,拍板才落盘。
- **交互输出口径**:估算/重定前先给一页复盘 memo，不要把 fetcher JSON、偏离诊断全文或完整新稿片段直接贴给用户。
  ```text
  我先把这次年度更新读成这样：
  1. 真实数据相对旧稿最大偏离在...
  2. thesis 兑现/失效的点是...
...
- **同权重定调文件只读当锚点 + 提示是否该刷新**,绝不覆写 `公司判断和最新观点.md` 或 `重要文件\` 下材料(那是 ka/分析师维护的)。
