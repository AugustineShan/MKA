...
**KA 特别标注**：最重要的是业务拆分理解及其历史。生成主拆分时优先使用分析师手写 thesis、BRKD/LOAD/Alphapai/reference 中更贴近业务的拆分；官方拆分默认是校验、桥表、父级 headline、收纳或兜底，不自动压过分析师拆分。所有进入本轮材料入口的业务数字必须有去处；不适合入模时，宁可作为副拆分/收纳保留。副拆分可以仅供参考，副拆分之和不要求严格等于营业收入；不得为了配平改数，也不得静默丢弃数字。

**财务费用特别标注**：不要把“利息净额不写”误读成“财务费用都不处理”。生息利息项、利息净额、`interest_expense_rate`、`cash_interest_rate` 交给 defaults/引擎；`other_fin_exp_abs` 是利润表外生·非利息项，`/ka` 费用数值门必须把它作为“非息财务费用”与销售费用率、管理费用率、研发费用率、税金及附加率同表同权重检测并反馈给用户：默认沿用 `Agent/financial_expense.yaml` / defaults，或在特殊企业、汇兑/手续费/贴息趋势变化被材料或分析师明示时裁决覆盖。
...
- 业务拆分历史覆盖与 OfficialBreakdowns 去处 → 编辑器 §5
- 数值门（收入→毛利→费用〔含非息财务费用 other_fin_exp_abs，同销售/管理/研发/税金及附加同表反馈〕→below-OP/税/少数股东→分红率→可选 BS/CF 覆盖→terminal）→ 编辑器 §6
- 年报查证纪律 → 编辑器 §7
...
## 落盘后机器门禁（official 稿必跑）

写完 official `.md` 落盘后，**必须**跑 `ka_assumption_lint` 校验 knobs 块硬结构契约，verdict=PASS 才算 /ka 完成：

```bash
py -m src.ka_assumption_lint "<刚落盘的 official 核心假设.md>" "<公司>/Agent/.modelking"
```

FAIL 即按报告提示修 `.md`（不是 patch 报告）。常见拦截：

- `COST_ABS_SIGN`：`family: cost_abs` 的 knobs 条目写了正数。减值项（资产减值/信用减值/其他资产减值）按附录A **存负值**（零允许）——引擎把这三个字段当**带符号损益调整**以 `+impact_adjustment` 加进 operate_profit，写正数会被当加项加回、静默虚增利润。`.md` 作者常误用"损失项写正数金额"惯例，此门在 /ka 阶段就拦下，避免到 /comp 才发现。改负值。
- `BAD_FAMILY`：块头 `compiler:` 用了非合法族名（自创/拼写错）。
- `MARGIN_MUTEX` / `UNIT` / `LEN` / `HORIZON` / `KNOBS_PARSE` / `KNOBS_MISSING`：见 `docs/knobs块契约.md`。

参考稿（`状态: reference`）不强制跑此门；official 稿必跑。
