...
### WIP 状态（未提交工作树）
- `src/yaml1_formula.py`（583 行，新增）+ `tests/test_yaml1_formula.py`（163 行，新增）：formula DAG 求值器，**8 个 formula 测试全绿**。
- `src/yaml1_cleaner.py` / `tests/test_yaml1_cleaner.py`：接入 formula。
- 6 份 docs/skills 同步修改（契约文档、compiler skill、数据流水线）。

### 文档 vs 现实的分歧（契约漂移，已发现）
- `docs/formula_DAG开发文档.md` 写"状态：已上线，受限执行器可用"。
- `docs/理解层_设计决策...md` 第 9 节"下一步"仍把 formula 标"延后/留槽"。
- 实际：formula 代码+测试是**未提交 WIP**，且全套测试未回绿。
- → **"已上线"是过度声明**。这恰是 formula 文档自己第 30 行警告的"半成品制造契约漂移"。
...
- **A. formula/DAG 到底算不算"上线"？** 收口标准是什么？我倾向：测试全绿 + 第二家异构公司跑通 formula 才改文档为"稳定"，否则文档先降级为"实验性/受限"。
