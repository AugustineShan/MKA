...
def auto_reconcile_annual_failure(db_path: Path, ticker: str, *, max_failures: int) -> int:
    ...
    try:
        ...
        print(
            f"annual_report_reconciler.py failed: {exc}. "
            "Inspect the recon output before trusting annual clean data.",
            file=sys.stderr,
            flush=True,
        )
        return 1
    ...
    if result.get("new_approved", 0) > 0:
        print(
            f"LLM approved {result['new_approved']} new override(s). "
