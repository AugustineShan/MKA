...
from . import cleaner as clean
from .annual_report_utils import (
    COMPANIES_DIR,
    PROJECT_ROOT,
...
from .config import PACKAGE_ROOT, load_env


DOCS_DIR = PACKAGE_ROOT / ".refs" / "tushare-docs"
KNOWN_DEFECTS_PATH = PACKAGE_ROOT / "knowledge" / "known_tushare_defects.json"
...
def run_reconciliation(
    ...
    *,
    ...
    approve_high_confidence: bool = True,
    only_year: str | None = None,
    only_code: str | None = None,
    no_llm: bool = False,
    output: str | Path | None = None,
    override_output: str | Path | None = None,
    verbose: bool = False,
) -> dict[str, Any]:
    """Run annual-report reconciliation for one company and return a result summary.

    This is the programmatic entry point used by ``tushareclean.pipeline``.
    It never mutates ``raw_tushare`` or the clean tables; it only writes
    evidence JSON and, when asked, an override file.
    """
    if write_overrides and no_llm:
        raise ValueError("write_overrides requires LLM evidence; remove no_llm.")
    ...
    if only_year:
        failures = [failure for failure in failures if failure.period == only_year]
    if only_code:
        failures = [failure for failure in failures if failure.code == only_code]

    total_failures = len(failures)
    failures = failures[: max(max_failures, 0)]
    field_docs = read_tushare_field_docs()
    known_defects = load_known_defects()

    analyses: list[dict[str, Any]] = []
    use_llm = not no_llm and not write_overrides
    for failure in failures:
        row = wide.loc[failure.period].to_dict()
        present = present_by_period.get(failure.period, set())
        if verbose:
            print(
                f"Analyzing {failure.code} {failure.period}: {failure.title}",
                file=sys.stderr,
            )
        analyses.append(
            analyze_failure(
    ...
    out = {
        ...
        "used_llm": use_llm or write_overrides,
        "used_llm_per_failure": use_llm,
        "requested_llm_override_confirmation": write_overrides,
        "analyses": analyses,
    }

    path = output_path(company_dir, output)
    write_json(path, out)
    ...
    if write_overrides:
        override_path = (
            Path(override_output).resolve()
            if override_output
            else company_dir / "recon" / "annual_report_overrides.json"
        )

        def _approved_count(data: dict[str, Any]) -> int:
            return sum(
                1
                for item in data.get("adjustments", [])
                if item.get("status") == "approved"
                and item.get("source") in clean.APPROVED_OVERRIDE_SOURCES
            )
        ...
        if override_path.exists():
            try:
                ...
                existing = {}
        before_approved = _approved_count(existing)

        rule_candidates = collect_rule_candidates(out)
        llm_confirmation = batch_llm_confirm_candidates(
            ticker, rule_candidates, known_defects=known_defects
        )
        ...
        new_approved = _approved_count(overrides) - before_approved
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"Wrote {result['evidence_path']}")
    print(f"Wrote {result['latest_path']}")
    if result["override_path"] is not None:
        print(
            f"Wrote {result['override_path']} "
            f"({result['new_approved']} new approved adjustment(s))."
        )
    print(
        f"Found {result['total_failures']} failure(s), "
        f"analyzed {result['failures_analyzed']}."
    )
    if args.fail_on_findings and result["total_failures"]:
        return 1
    return 0
