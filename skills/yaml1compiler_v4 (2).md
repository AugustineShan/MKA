# YAML1 Compiler — Skill v4

> 本版相对 v3 的变化:修掉一轮多模型实测暴露的七处问题,核心是消除会**主动教错模型**的自相矛盾指令。
> 1.（致命）拆开"财务费用":利息净额引擎倒算不写、其他财务费用外生项必写且必须嵌 `income.financial_expense` 下(此前 §8/附录A/范例三处一刀切"一字不写",听话的模型会把外生项整个丢掉)。
> 2. 速查表补齐 `vol_price_margin` 与 `abs` 两族行(此前只有 vol_price/growth,产品线级拨毛利率或绝对值族的公司查表查不到)。
> 3. §5.2 补齐 `abs` 族完整定义(此前盘点清单列了五族,正文从未定义 abs)。
> 4. 附录B范例补 `forex_gain`、`credit_impa_loss`、`financial_expense.other_fin_exp_abs` 三个常见字段(此前"完整范例"系统性漏掉)。
> 5. 把减值符号的血泪结论固化进速查表(`cost_abs.*` 存负值、与 .md 同号、引擎同号减项;此前留成"自己去核"的开放状态)。
> 6. §0.1 给"6 年 fade 至 2038"标注为反例并前向引用 §7.3。
> 7. 收敛族清单与正文定义,使盘点的五族与正文一一对应。
>
> v3 主线(认知主线"这是一次什么样的翻译"、三积木、leaf 三件套、三条去向、自审)与 v3.x 的 hold_paths 边界、未铺三段式默认值,全部保留。
...
### 0.1.1 注意力铁律:yaml1 是 Excel 的结构化抽象
...
| Excel 公式关系 | `decomposition` / `factor_product` / `growth` / `abs` / margin fold | 模板层无状态、无引用、无 DAG |
| 滞后链、DAG、分段函数、中间变量复用 | 不生成;收尾报告举旗 | 当前 formula 引擎未实现 |

一句话:**数字只是材料,旋钮和公式才是骨架,历史是证据。** 你要翻的是这三者的结构关系,不是把看见的格子逐个塞进 YAML。
...
- 收入拆几条线?每条什么可执行模板(`factor_product` / `growth` / `abs`,以及兼容旧名 `vol_price` / `vol_price_margin`)?有没有下钻到产品号(嵌套)?若遇到滞后/DAG/分段等 formula 形态,只举旗,当前不生成。
...
### 4.3 `formula`(算法模式:当前未实现,只能举旗)

**当前下游 `yaml1_cleaner.py` 没有 formula/DAG 求值器。** 所以 formula、`bridge`、`ratio_to_driver`、跨期递推、分段函数、`ref`/`lag_ref` 中间变量复用,都只能在报告里举旗,**不得生成进 yaml1**。绝大多数常见收入线不要走 formula,先尝试可执行模板:`factor_product`(n 因子连乘)、`growth`、`abs`。

只有当 `.md` 明确写出模板装不下的滞后链/DAG/分段规则时,你在收尾报告里写清"需要 formula 引擎,当前未实现",交人处理。不要把它伪装成新族名,也不要自行编公式。
...
上面只是未来 formula 层的设计草图,**不是当前可执行 YAML1**。当前 compiler 输出里出现 `kind: formula` 会被 cleaner 拒绝。
...
照 `.md` 骨架声明的模板给对应旋钮,满数组。`revenue_family` 是这条 leaf 的算法声明,**只能从当前 cleaner 支持的有限集合里选**:`factor_product` / `growth` / `abs`,以及旧样本兼容名 `vol_price` / `vol_price_margin`。不要自创族名;`formula` 当前未实现,见 §4.3。
...
换一家公司,老板可能在产品线级拨毛利率。此时不要写顶层 `income.gpm`;把每个 revenue leaf 的 `knobs.margin` 写满数组,由 cleaner 做 `sum(leaf_revenue × margin) / sum(leaf_revenue)` 折成整体 `income.gpm`。**二选一硬规则:**没有任何 leaf margin → 才允许顶层 `income.gpm` 手拍;只要任何 leaf 有 margin → 禁止顶层 `income.gpm`,且所有收入 leaf 都必须有 margin。部分 leaf margin、或 leaf margin + 顶层 `income.gpm` 同时出现,都是 over-determined,必须举旗。
...
- **财务费用 · 利息净额** → 不在 yaml1。引擎按现金/负债余额倒算,`#VALUE!` 是设计如此,一个字不写。**但"财务费用"不是铁板一块**:利息净额是引擎倒算的(不写),而**其他财务费用(外生、非利息项,如手续费等)是 .md 可能拍的外生绝对值,必须写**。.md 里若给了这个外生项(它在字典/cleaner 里叫 `other_fin_exp_abs`),就落成 knob:
  ```yaml
  income.financial_expense.other_fin_exp_abs:
    ...
    src: "#其他财务费用(外生·非利息)"
  ```
  **🔴 命名空间硬契约:必须嵌在 `income.financial_expense` 下。** cleaner 是 `income.get("financial_expense")` 进去取的——写到顶层 `financial_expense.*` 或别处会被**静默丢弃**,该外生项落 0,没有任何报错。这正是"利息净额不写"被误读成"整个财务费用都不写"会丢掉的判断:**拆开记——利息净额不写,其他财务费用外生项必写、且必须嵌 income.financial_expense 下。**
...
| 装不进任何族的算法线 | `formula` | 暂不生成 | 当前 cleaner 未实现 formula/DAG;收尾报告举旗 | 不得写进 yaml1 |
...
# 财务费用:利息净额引擎倒算不写;其他财务费用(外生·非利息)必写,且必须嵌 income.financial_expense 下
...
**这是一次翻译:把人话的、按业务线组织的、带判断且宽容的 `核心假设.md`,翻成机器话的、按路径组织的、无损零容忍的 `yaml1`。翻译要弥合四个语法差异——组织维度、深度语义、符号口径、歧义容忍;前三个是形变,照翻;第四个是信息缺口,举旗。你逐句把 A 类判断认领到 defaults.yaml 真实路径、以正确 kind 和正确符号落地、旋钮摊满数组、非标拆分镜像成 ≤2 子树、收入 leaf 只用当前可执行模板(`factor_product`/`growth`/`abs`/兼容旧名),三段式带 hold/fade 分组、每条 leaf 给齐 base/knobs/history;B 类把分线历史落进各 leaf history、收纳区独立成块进 stash,多年序列一个不丢。你不算账、不重算、不折叠、不展开 fade、不发明路径、不逆向公式、不把 B 类塞进 note;formula/DAG 当前未实现,只能举旗,不得生成。能翻就翻、翻不了举旗、绝不猜;收尾对着盘点清单校对(双射 + B 类完整性 + unaligned/待核 + 主动覆盖人话回读 + 结构异常硬停),交下游 清洗yaml1.py 折叠展开 resolve、calc.py 纯算 DCF。**
