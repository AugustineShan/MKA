...
terminal:
  ...
  perpetual_growth: 0.02        # 体系纪律默认值,固定 0.02,不要随意换数
```

唯一固定的一点:`perpetual_growth` 默认 **`0.02`**(本体系锚长期通胀的纪律值),别填别的数。其余四个(to_year/kind/fade_paths/hold_paths)就按上面照抄。.md 里"本轮未铺/待定"的痕迹保留在 `src`/`note` 里即可,无需额外标记。
