...
- 哪些是**主动覆盖**?——只认源文行尾 `[主动覆盖]` 标记的线(逆券商 / 参数化翻转 / 异常值常态化 / 查证类拐点);**compiler 不自己判定分析师意图**,源文没标的不进主动覆盖回读。这些收尾要单独人话回读。
...
> **这是差异 4(歧义)的教科书场景。** .md 的措辞常自相矛盾,例:"2032 起 6 年 fade 至 2038"——"6 年"(2032–2037)与"至 2038 / 2038 起永续"在端点上不自洽(含端 2032–2038 是 7 年)。**处理四步:(1) 识别出这是源语言的信息缺口,不是你能机械翻的形变;(2) 取一个内部自洽解(如:让"6 年"成立 → fade 2032–2037、2038 首个永续年);多个自洽解冲突时默认偏向保守——取更短 fade 年限 / 更低 `target_growth`;(3) 在该字段加 `# to_year 语义待核` 注释;(4) 在收尾报告里写明歧义本身 + 你取的解 + 偏保守的偏向,交人裁定。** 绝不静默选一个字面值放行——因为你选的可能恰好是 .md 写错的那一半,而机器不会报错。
...
terminal:
  ...
  perpetual_growth: 0.025       # 默认 0.025,跟随 defaults.yaml 的 model.terminal_growth
```

唯一固定的一点:`perpetual_growth` 默认 **`0.025`**——**跟随 `defaults.yaml` 的 `model.terminal_growth`**(当前为 0.025);若 defaults 该值变更,以 defaults 为准,别填别的数。若 .md 没铺三段式，默认不写 `target_growth` 或 `path_targets`，保持旧语义；默认 `hold_paths: [income.gpm]` 只是为了防止整体毛利率被旧语义误推到永续增速,若 .md 明示 GPM 到某个目标值,必须改用 `path_targets`。`.md` 里"本轮未铺/待定"的痕迹保留在 `src`/`note` 里即可,无需额外标记。
...
5. **主动覆盖回读:** 把源文行尾标 `[主动覆盖]` 的线(参数化翻转、逆券商、异常值常态化、查证类拐点——历史回测够不着、又故意不照搬券商,下游没有客观闸接得住)单独渲染成一张紧凑人话表,摆给老板扫一眼。**只认源文标记,不自己判定意图**;源文该标却没标的,在报告里标"疑似主动覆盖未标记"交人。这是那条窄缝唯一的真值裁判。
