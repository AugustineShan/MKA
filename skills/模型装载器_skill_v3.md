...
- overview 和逐段确认都控制在一个可读屏幕左右；展示优先用表格，把模型历史值和模型预测值放在同一张主表里，一次交互尽量只给一张表。
- 用 1-3 句结论 + 一张主表 + 一个确认问题；把 source range、装载方式、风险/待确认点压进表格列。除非行列太宽或不同口径混放会误导，不要一段里拆成多张碎表。
...
关键旋钮（2025-2028）：
| 线 | 模型历史实际 | 模型预测值 | source range | 我会怎么装 | 风险/待确认 |
|---|---|---|---|---|---|
| 中高端 | 历史收入/销量/吨价按模型历史列搬运 | 销量 yoy 20/16/12/8；吨价 yoy 6.0/5.6/5.2/4.8 | 年度和半年度!xx:xx | factor_product | 按原模型保留 |
| 普通 | 历史收入/销量/吨价按模型历史列搬运 | 销量 yoy -12.35 flat；吨价 yoy 4.75/4.5/4.25/4.0 | 年度和半年度!xx:xx | factor_product | -12.35% flat 偏强，但属于 load-vintage |
| 其他 | 历史收入按模型历史列搬运 | 收入 yoy 5 flat | 年度和半年度!xx:xx | growth | 显式期截到 2028，2029-2031 和 2031 断裂列不装 |
