...
`/load` 完整继承核心纪律 A1-A7，按核心假设源语言 B 输出。本文只写 `/load` 独有边界和模型公式层读法。
...
- 费用：销售、管理、研发、财务费用的历史特征和模型处理方式。
- below-OP 与税：大额特殊项、税率、投资收益、减值等模型处理方式。
- 中期：显式期、衰减期、终值/永续增长的初步理解。
...
- 税金及附加、销售、管理、研发：费用率族。
- 财务费用拆两块：利息净额跳过；其他财务费用外生项单独看。
- below-OP：投资收益、其他收益、公允、资产处置、减值、营业外收支逐项判断。
- 有效税率、少数股东比例：若模型有显式旋钮则写，派生结果不当旋钮。

### 中期

写显式期、衰减期、`fade_paths`、`hold_paths`、永续增长点。不要展开 fade 逐年序列。
