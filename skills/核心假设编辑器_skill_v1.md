...
上游产物都是 markdown 化后的候选理解：`Agent业务讨论.md`、`核心假设参考load_*.md`、`核心假设参考alphapai_*.md` 或 factpack/reference。`/ka` 是唯一裁决器；它不是把候选稿合并，而是把候选 markdown、同权重判断材料和 `/init` 事实裁成一份 official 源文。
...
若输入里只有同权重判断材料或旧正式稿，没有 BRKD 产物、完成的 LOAD 产物或其他 KA 参考稿区 reference 候选，停止。`/ka` 不能凭空生成业务骨架。
...
4. KA 参考稿区 `核心假设参考*.md`（brkd/load/alphapai 统一命名 `核心假设参考{来源}_YYYYMMDD.md`）：reference/draft 候选，包括 Alphapai-load 输出；只作候选理解，不作拍板结果。其 `## 待 /ka 裁决清单` 是晋升前议程，不是正式判断；每条事项必须被裁成入模、收纳、缺口或丢弃。
