...
它只管“怎么做才不丢、不漂、不偷算”。它不定义输出块格式；输出语法见 `skills/核心假设源语言_skill_v1.md`。若分不清该读哪份契约，先看 `docs/MKA规则导航图.md`；那是索引，不是新纪律。
...
人工筛选门优先于材料吞吐：markdown 存储区是 cache，不是证据入口；看见 markdown 不等于必须吸收。只有被人工放入同权重判断材料、BRKD/LOAD/Alphapai 完成产物、KA 目录顶层 markdown 或 `/init` 事实索引的内容，才进入本轮裁决。

KA 目录固定指 `companies\{公司}\Skills素材包\KA（ALPHAPAI拆出来的东西放在这里）\`。该目录顶层所有 `*.md` 都是 `/ka` 人工筛选入口；`核心假设参考*.md` 按候选稿裁决，其他 markdown 按信息指引读取，不要求伪装成 reference。

入口窄，收纳宽：人工筛选门只限制读取范围，不削弱 A1/A2。已进入本轮的材料里，有复盘价值但暂不入模的信息宁可进收纳区/stash，不得为了减负丢掉；丢弃只用于重复、无关、越界或低可信且无复盘价值的信息，并写明理由。
