...
交互风格必须像排程讨论会,不是 YAML/schema 审查。默认先给一页会议 memo:

```text
我读完 DA facts 后,先给判断:
1. 存量资产结构和折旧基线是...
2. 未来真正要拍的是扩张 capex 和转固节奏...
3. 我建议的默认排程是...
...
这个默认排程方向你认吗?认了我再逐项写入 da_schedule.yaml。
```

不要在聊天里默认贴完整 `da_facts_latest.json`、完整 `da_schedule.yaml` schema 或所有资产明细；用户要核某个分类或来源时再展开。完整来源、decision_log 和 gaps 必须写进 `da_schedule.yaml`。
