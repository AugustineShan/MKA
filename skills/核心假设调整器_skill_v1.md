...
`/adj` 两种模式都必须先读同权重判断材料：`公司判断和最新观点.md` + `重要文件/` 顶层材料（经 `src.ka_prepare` 进入最高权重 `markdown存储区/`）。`重要文件/` 常放最重要、最新的会议纪要，凡读公司判断必须一起读；quick 若发现小改方向与这组材料冲突，先提示用户确认。

交互风格继承核心纪律 A4：quick 是“手术单”，incremental 是“增量影响 memo”。聊天里先输出你对用户意图的理解、影响范围和建议动作；确认后才落盘。不要把完整 knobs/yaml1 path 清单机械贴满屏，除非用户要求核路径。

判断原则（决策树定义在 `/adj` 启动器，此处只给示例对照，不重复判据）：

- “把毛利率稍微提一提”“销售费用率降 0.5pct”“2026 收入增速上调到 5%” -> 能映射到已有 knobs 且不碰结构 -> quick。
- “新开了一个渠道”“竞争格局变了”“这份纪要说明管理层目标变化” -> 带新业务理由/新材料/结构变化 -> incremental。
...
**白名单与禁止清单的单一真源是 `docs/旋钮白名单与结构判定.md`**（§一 quick 可拨白名单、§二 quick 禁止清单）。本 skill 不重抄，只守其约束：

- 白名单不可加宽；新增任何 path、结构、slug、family、horizon 或 fade 编辑都不是 quick，转 `/adj incremental` 或重走 `/comp`。
- 结构改动走 §三结构判定表（可观测特征 → 处理路径），不再用"参数化翻转/推翻裁决基础"等主观词。
...
确定性强制（A4 闸门）：除人工逐条核对外,落盘后必须跑 `py -m src.yaml1_fidelity_check "<yaml1>" "Agent\defaults.yaml" "<核心假设.md>"`。exit 1（BLOCK,md↔knobs↔yaml1 双射 FAIL）→ 停止,md 赢,回 `/comp` 重编译,**不跑 forecast**;exit 0（PASS）才跑 DCF。详情读 `Agent\.modelking\yaml1_fidelity_report.json`。
...
```text
Skills素材包\最高权重材料-放Agent最应对齐的材料\markdown存储区\
...
- 找不到正式核心假设。
- 找不到 `公司判断和最新观点.md` 或同权重判断材料 prepare 失败。
- 找不到最新 yaml1。
- 用户请求映射不到已有 knobs。
- quick 请求需要新增结构或改参数化。
- quick 请求改变 horizon、terminal fade 长度或 `terminal.fade.target_growth`。
