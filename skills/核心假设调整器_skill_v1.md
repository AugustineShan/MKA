...
- 已存在收入 leaf:`projection.values[i]`(factor_product/vol_price 族,因子级) 或 `knobs.revenue_yoy[i]`/`revenue_abs[i]`(growth/abs 族,segment 级)
- 已存在 below-OP 绝对值 path 的 `values[i]`(`income.operating_adjustments_abs.{field}` 或 `income.below_line_abs.{field}`,以实际 yaml1 为准)
